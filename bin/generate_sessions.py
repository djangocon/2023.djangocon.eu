...
def get_talks(refetch=False):
    ...
    if selections_path.exists() and refetch is False:
        ...
        submissions = {
            ...
            if _relevant_session_type(result)
        }
...
def get_speakers(talk_codes, refetch=False):
    ...
    if speakers_path.exists() and refetch is False:
        ...
        speakers = {
            result["code"]: result
            for result in resp["results"]
            if any(sub in talk_codes for sub in result["submissions"])
        }
...
def update_info(talk, speakers):
    formatted = {
        ...
        "type": talk["submission_type"]
        if isinstance(talk["submission_type"], str)
        else talk["submission_type"]["en"],
        "state": talk["state"],
    ...
    formatted["name_slug"] = (
        re.sub(r"\W+", "-", first_speaker["name"]).strip("-").lower()
    )
    return formatted
...
if __name__ == "__main__":
    ...
    """
    parser.add_argument(
        "--refetch", "-r", action="store_true", help="Fetch schedule info from pretalx"
    )
